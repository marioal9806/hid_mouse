...
void APP_ProcessSwitchPress(BSP_SWITCH pressed_switch)
{
    /* This function checks if the switch is pressed and then
     * debounces the switch press*/
    if(BSP_SWITCH_STATE_PRESSED == (BSP_SwitchStateGet(pressed_switch)))
    {
        while(BSP_SWITCH_STATE_PRESSED == (BSP_SwitchStateGet(pressed_switch)));
...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state )
    {
        ...
        case APP_STATE_MOUSE_EMULATE:
            // emulateMouse == 0 -> funcion 1
            // emulateMouse == 1 -> funcion 2
            
            // Check if we're toggling between functions.
            APP_ProcessSwitchPress(APP_USB_SWITCH_1);
            ...
            // Check if a click was sent before
            // If so, send a packet with the clicks released
            // Could be the first if, or it could include a break statement to
            // jump to the next iteration
            if(appData.isMouseTrapped) {
            ...
            else if(appData.emulateMouse)
            {
                // funcion 2 de los botones
                
                // leer boton 2
                // si fue presionado, enviar movimiento mouse a la derecha
                APP_ProcessSwitchPress(APP_USB_SWITCH_2);
                if(appData.isSwitchPressed)
                {
                    appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                    appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
                    appData.xCoordinate = 10;
                    appData.yCoordinate = 0;
                    appData.isSwitchPressed = false;
                    appData.isMouseTrapped = true;
                }
                
                // leer boton 3
                // si fue presionado, enviar movimiento mouse a la izquierda
                APP_ProcessSwitchPress(APP_USB_SWITCH_3);
                if(appData.isSwitchPressed)
                {
                    appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                    appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
            ...
            { 
                // funcion 1 de los botones
                
                // leer boton 2
                // si fue presionado, enviar click derecho
                APP_ProcessSwitchPress(APP_USB_SWITCH_2);
                if(appData.isSwitchPressed)
                {
                ...
                // leer boton 3
                // si fue presionado, enviar click izquierdo
                APP_ProcessSwitchPress(APP_USB_SWITCH_3);
                if(appData.isSwitchPressed)
                {
                    appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
